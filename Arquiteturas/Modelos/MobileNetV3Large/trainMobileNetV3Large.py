...
def get_model():
    # Load pretrained MobileNetV3-Large and replace classifier head for binary output
...
for run_idx, seed in enumerate(SEEDS):
    ...
    np.save(f"fpr_mobilenetv3large_run{run_idx + 1}.npy", fpr)
    np.save(f"tpr_mobilenetv3large_run{run_idx + 1}.npy", tpr)

    # Plot and save confusion matrix
    cm   = confusion_matrix(y_true, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=classes)
    disp.plot(cmap="Greens")
    plt.title(f"Confusion Matrix – MobileNetV3-Large – Run {run_idx + 1} (seed={seed}) – Acc: {acc:.2f}%")
    plt.tight_layout()
    fname = f"matriz_mobilenetv3large_run{run_idx + 1}.png"
    plt.savefig(fname, dpi=300)
    plt.close()
    print(f"  Confusion matrix saved: {fname}")

    # Save model weights
    torch.save(net.state_dict(), f"mobilenetv3large_run{run_idx + 1}_seed{seed}.pth")
    print(f"  Model saved: mobilenetv3large_run{run_idx + 1}_seed{seed}.pth")
