...
save_dir   = "/content/drive/MyDrive/Colab Notebooks/DatasetKagle/modelos_resnet50"
...
for run_idx, seed in enumerate(SEEDS):
    ...
    with torch.no_grad():
        for images, labels in testloader:
            ...
            outputs  = net(images)
            probs    = F.softmax(outputs, dim=1)[:, 0]  # probability for class 0
    ...
    np.save(f"{save_dir}/fpr_resnet50_run{run_idx + 1}.npy", fpr)
    np.save(f"{save_dir}/tpr_resnet50_run{run_idx + 1}.npy", tpr)

    # Plot and save confusion matrix
    cm   = confusion_matrix(y_true, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=classes)
    disp.plot(cmap="Greens")
    plt.title(f"Confusion Matrix – ResNet-50 – Run {run_idx + 1} (seed={seed}) – Acc: {acc:.2f}%")
    plt.tight_layout()
    fname = f"{save_dir}/matriz_resnet50_run{run_idx + 1}.png"
    plt.savefig(fname, dpi=300)
    plt.close()
    print(f"  Confusion matrix saved: {fname}")

    # Save model weights
    model_path = f"{save_dir}/resnet50_run{run_idx + 1}_seed{seed}.pth"
