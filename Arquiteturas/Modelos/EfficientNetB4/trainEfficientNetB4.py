...
for run_idx, seed in enumerate(SEEDS):
    ...
    np.save(f"{save_dir}/fpr_efficientnetb4_run{run_idx + 1}.npy", fpr)
    np.save(f"{save_dir}/tpr_efficientnetb4_run{run_idx + 1}.npy", tpr)
    ...
    plt.tight_layout()
    fname = f"{save_dir}/matriz_efficientnetb4_run{run_idx + 1}.png"
    plt.savefig(fname, dpi=300)
    plt.close()
    print(f"  Confusion matrix saved: {fname}")

    # Save model weights
    model_path = f"{save_dir}/efficientnetb4_run{run_idx + 1}_seed{seed}.pth"
